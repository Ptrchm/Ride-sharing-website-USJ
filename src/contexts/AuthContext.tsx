...
import { getMe, login as apiLogin, signup as apiSignup, logout as apiLogout, isAuthenticated, logEvent, updatePassengerStatus } from "@/services/xanoApi";
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const login = async (email: string, password: string) => {
    ...
    await logEvent("login", { email: u.email, user_id: u.id });
    setLoading(false);
    return u;
  };
  ...
  const signup = async (name: string, email: string, password: string) => {
    ...
    const u = await fetchMeAndSet();
    await logEvent("signup", { email: u.email, user_id: u.id, name: u.name });
    setLoading(false);
    return u;
  };

  const logout = () => {
    void updatePassengerStatus({ isLookingForRide: false })
      .catch(() => undefined)
      .finally(() => {
        apiLogout();
        setUser(null);

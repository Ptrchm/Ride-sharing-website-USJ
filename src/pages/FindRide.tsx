...
import { getAcceptedRides, getRideRequests, getRoutes, createRideRequestWithChecks } from "@/services/xanoApi";
import { useAuth } from "@/contexts/AuthContext";
import { toast } from "@/hooks/use-toast";
import { Search, ArrowRight } from "lucide-react";
import { formatDepartureDate, formatDepartureTime } from "@/utils/dateTime";
...
const isRouteAvailable = (route: any) => {
  const now = Date.now();
  const departMs = toEpochMs(route?.departure_time);
...
export default function FindRide() {
  ...
  useEffect(() => {
    const computeRestrictions = async () => {
      ...
      const routeDepartMs = (r: any) => toEpochMs(r?.departure_time);
      ...
      if (myUpcomingCreated.length > 0) {
        ...
        setJoinBlocked({
          reason: `Vous avez un trajet à venir en tant que conducteur (${formatDepartureDate(untilMs)} à ${formatDepartureTime(untilMs)}). Vous pourrez rejoindre d'autres trajets après ce départ.`,
          untilMs,
        });
        return;
      }
      ...
      try {
        ...
        if (untilMs > 0) {
          setJoinBlocked({
            reason: `Vous avez déjà une réservation en cours (${formatDepartureDate(untilMs)} à ${formatDepartureTime(untilMs)}). Vous pourrez rejoindre un autre trajet après ce départ.`,
            untilMs,
          });
          return;
        }
  ...
  const handleSearch = () => {
    ...
    if (date) {
      ...
      result = result.filter((r: any) => {
        ...
        const rideDate = new Date(toEpochMs(r.departure_time));

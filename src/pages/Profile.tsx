...
export default function Profile() {
  ...
  const handleRequestAction = async (requestId: number, status: "accepted" | "declined") => {
    ...
    try {
      if (status === "accepted") {
        // Backend handles: ride_request status + accepted_ride + seat decrement.
        await createAcceptRide({ ride_request_id: requestId });
      } else {
    ...
    unseen.slice(0, 3).forEach((r) => {
      ...
      toast({
        ...
        description:
          ...
          (typeof seatsBooked === "number" ? ` Places réservées: ${seatsBooked}.` : "") +
